...
# The line below creates a time vector that sequences values between 0 and 8 by increments of 1
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates a storage matrix with the number of rows and columns based on our previous vectors.
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below sets the first row of our matrix equal to our pre-determined initial values.
proj_year[1, ] <- fish_year0

# The for loop below.multiplies the coefficient matrix by the values from the previous year, then sets the new year's values in our storage matrix.
